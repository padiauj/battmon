...
def get_battery_states():
    ...
    for supply in supplies:
        ...
        if (type == "Battery"):
            battery_id =  "_".join([read_path(os.path.join(supply_path, bnf)) for bnf in battery_name_files])
            battery_states[battery_id] = {}
            for bdf in battery_data_files:
                battery_states[battery_id][bdf] = read_path(os.path.join(supply_path, bdf))
...
def log_battery_state():
    ...
    for battery in states:
        ...
        fields = [time, states[battery]["status"][0].upper(), states[battery]["capacity"]]
    ...
    for logfile in logfiles:
        ...
        for row in reader:
            ...
            if (timestamp >= cutoff):
                ...
                time.append(datetime.datetime.fromtimestamp(timestamp / 1000.0))
...
class Window(QtGui.QDialog):
    def __init__(self, parent=None):
        ...
        timeframe_gbox = QtGui.QGroupBox("Time") 
        ...
        for battery in states:
            ...
            label += " (" + states[battery]["capacity"] + "%, " + states[battery]["status"] + ")"
